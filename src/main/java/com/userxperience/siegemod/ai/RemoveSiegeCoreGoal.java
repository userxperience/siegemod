...
public class RemoveSiegeCoreGoal extends MoveToBlockGoal {
    private final ModBlocks blockToRemove;
    private final Mob removerMob;
    private int ticksSinceReachedGoal;
    private static final int WAIT_AFTER_BLOCK_FOUND = 20;

    public RemoveSiegeCoreGoal(ModBlocks pBlockToRemove, PathfinderMob pRemoverMob, double pSpeedModifier, int pSearchRange) {
    ...
    private BlockPos getPosWithBlock(BlockPos pPos, BlockGetter pLevel) {
        if (pLevel.getBlockState(pPos).is((HolderSet<Block>) this.blockToRemove)) {
            return pPos;
        } else {
            BlockPos[] ablockpos = new BlockPos[]{pPos.below(), pPos.west(), pPos.east(), pPos.north(), pPos.south(), pPos.below().below()};

            for(BlockPos blockpos : ablockpos) {
                if (pLevel.getBlockState(blockpos).is((HolderSet<Block>) this.blockToRemove)) {
                    return blockpos;
                }
            }
    ...
    protected boolean isValidTarget(LevelReader pLevel, BlockPos pPos) {
        ...
        if (chunkaccess == null) {
            ...
            return chunkaccess.getBlockState(pPos).is((HolderSet<Block>) this.blockToRemove) && chunkaccess.getBlockState(pPos.above()).isAir() && chunkaccess.getBlockState(pPos.above(2)).isAir();
        }
    }
}
